...
class Text extends Tret
{
	...
	public $rules = array(
		...
		'no_repeat_words' => array(
				...
				'replacement' 	=> array(
					'\1',
					'$m[1].($m[7] === EMT\Lib::strtolower($m[3]) ? $m[2] : $m[2].$m[5].$m[6] )',
					)
			),
		'paragraphs' => array(
	...
	{
		$this->_text = $this->preg_replace_e('/(\<\/' . self::BASE64_PARAGRAPH_TAG . '\>)([\r\n \t]+)(\<' . self::BASE64_PARAGRAPH_TAG . '\>)/mse', '$m[1].EMT\Lib::iblock($m[2]).$m[3]', $this->_text);
...
/**PYTHON
    ...
    def do_paragraphs(self, text):
        text = EMT\Lib.str_replace(u"\r\n",u"\n",text)
        text = EMT\Lib.str_replace(u"\r",u"\n",text)
        text = u'<' + BASE64_PARAGRAPH_TAG + u'>' + text.strip() + u'</' + BASE64_PARAGRAPH_TAG + u'>'
        text = self.preg_replace('/([\040\t]+)?(\n)+([\040\t]*)(\n)+/e', '(u"" if m.group(1) is None else m.group(1))+u"</" + BASE64_PARAGRAPH_TAG + u">"+EMT\Lib.iblock(m.group(2)+m.group(3))+u"<" +BASE64_PARAGRAPH_TAG + u">"', text)
    ...
    def build_paragraphs(self):
        ...
        if(( r != -1) and (p != -1)):
            ...
            beg = EMT\Lib.substr(self._text,0,r);
            end = EMT\Lib.substr(self._text,p+len(u'</' + BASE64_PARAGRAPH_TAG + u'>'))
            self._text = (self.do_paragraphs(beg)+ u"\n" if beg.strip() else u"") +u'<' + BASE64_PARAGRAPH_TAG + u'>'+EMT\Lib.substr(self._text,r + len(u'<' + BASE64_PARAGRAPH_TAG + u'>'),p -(r + len(u'<' + BASE64_PARAGRAPH_TAG + u'>')) )+u'</' + BASE64_PARAGRAPH_TAG + u'>'+( u"\n"+self.do_paragraphs(end) if end.strip() else u"")
        else:
            self._text = self.do_paragraphs(self._text)

    def build_brs(self):
        self._text = self.preg_replace('/(\<\/' + BASE64_PARAGRAPH_TAG + '\>)([\r\n \t]+)(\<' + BASE64_PARAGRAPH_TAG + '\>)/mse', 'm.group(1)+EMT\Lib.iblock(m.group(2))+m.group(3)', self._text);

        if (not re.match('\<' + BASE64_BREAKLINE_TAG + '\>', self._text)):
            self._text = EMT\Lib.str_replace("\r\n","\n",self._text)
            self._text = EMT\Lib.str_replace("\r","\n",self._text)
