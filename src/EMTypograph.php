...
class EMTypograph extends Base
{
    public $trets = [
        Tret\Quote::class,
        Tret\Dash::class,
    ...
    protected $all_options = [
        ...
        'Etc.entity_to_numeric_convert' => ['description' => 'Преобразовывать html-сущности в числовые html-сущности', 'selector' => '*', 'setting' => 'donumericentity', 'disabled' => true],
    ...
    public function do_setup($name, $value)
    {
        ...
        if ($name == 'OptAlign.layout') {
            if ($value == 'style') {
                $this->set_tag_layout(Lib::LAYOUT_STYLE);
            }
            if ($value == 'class') {
                $this->set_tag_layout(Lib::LAYOUT_CLASS);
            }
        }
    }
